...
class Exporter:
    ...
    def __init__(self):
        ...
        self.style_plan_name = xlwt.easyxf("borders: top thin, bottom thin, left thin, right thin; align: horiz centre;")
        ...
        align_centre = 'align: horiz centre;'
        align_right = 'align: horiz right;'

        self.style_material_header = xlwt.easyxf(align_centre + material_borders)

        self.style_material_left = xlwt.easyxf(align_left + material_borders)
        self.style_material_centre = xlwt.easyxf(align_centre + material_borders)
        self.style_material_right = xlwt.easyxf(align_right + material_borders)
    ...
    def export_detail(self, order_id):
        ...
        for section in sections:
            ...
            sheet.write(cur_row, cur_col, '小计', self.style_material_centre)
            cur_col += 1
            sheet.write(cur_row, cur_col, section['final_price'], self.style_material_right)
            cur_row, cur_col = self.__row_complete(sheet, cur_row, cur_col)

            cur_row, cur_col = self.__row_complete(sheet, cur_row, cur_col)

        sheet.write(cur_row, cur_col, u'总计(' + order['footer'] + ')', self.style_material_centre)
        cur_col += 1
        sheet.write(cur_row, cur_col, order['final_price'], self.style_material_right)
        cur_row, cur_col = self.__row_complete(sheet, cur_row, cur_col)
